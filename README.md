...
To inspect external publication readiness after a release candidate passes, run:

```bash
make publication-readiness
```

The readiness report checks local release files plus GitHub repository visibility, GitHub Pages, GitHub Release, Zenodo DOI, and OSF mirror state. It is not part of `make release-candidate` because public visibility, licensing, DOI minting, and OSF publication are owner/account-side actions.

After Zenodo's GitHub integration is enabled for this repository, publish the prepared draft release with:

```bash
make publish-release-after-zenodo
```

The release publisher refuses to run until Zenodo integration is visible through GitHub hooks, unless the Zenodo dashboard has been verified manually and `scripts/publish_release_after_zenodo.py --zenodo-dashboard-verified --publish` is used directly. After Zenodo mints the DOI and the OSF project is public, backfill archive links with:

```bash
make backfill-archive-links DOI="10.5281/zenodo.<record-id>" OSF_URL="https://osf.io/<project-id>/"
```

The account-side sequence and metadata fields are recorded in `notes/publication_handoff.md`.
