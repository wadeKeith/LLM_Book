...
\paragraph{Chapter contract.} The reader should leave this chapter able to compare LLaMA-class architectural choices against the basic GPT contract from the previous chapter, quantify their training and serving consequences, and ask which evidence is needed before treating a design as an improvement.

\section{What Makes a Decoder LLaMA-Class}
\index{LLaMA}\index{modern decoder}\index{decoder-only Transformer}
...
Figure~\ref{fig:llama-class-block} summarizes the coupled defaults that make a decoder LLaMA-class in practice.

\begin{figure}[t]
\centering
\resizebox{0.98\textwidth}{!}{%
\begin{tikzpicture}[node distance=7mm and 8mm]
\node[llmblock, text width=2.1cm] (x) {Residual\\stream};
...
\node[llmblock, right=of gqa, text width=2.1cm] (y) {Post-attn\\stream};
\node[llmblock, below=13mm of x, text width=2.1cm] (y2) {Post-attn\\stream};
\node[llmblock, right=of y2, text width=2.0cm] (r2) {RMSNorm};
\node[llmblock, right=of r2, text width=2.2cm] (ffn) {SwiGLU\\or MoE FFN};
\node[llmblock, right=of ffn, text width=2.1cm] (out) {Next residual\\stream};
\draw[llmarrow] (x) -- (r1);
\draw[llmarrow] (r1) -- (rope);
\draw[llmarrow] (rope) -- (gqa);
\draw[llmarrow] (gqa) -- (y);
\draw[llmarrow] (y.south) -- (y2.north);
\draw[llmarrow] (y2) -- (r2);
\draw[llmarrow] (r2) -- (ffn);
\draw[llmarrow] (ffn) -- (out);
\draw[llmdash] (x.north) .. controls +(0,0.85) and +(0,0.85) .. (y.north);
\draw[llmdash] (y2.south) .. controls +(0,-0.85) and +(0,-0.85) .. (out.south);
\node[llmnote, below=8mm of ffn, text width=0.70\textwidth] {The LLaMA-class recipe is a set of coupled defaults. Changing GQA changes serving memory; changing RoPE changes long-context behavior; changing SwiGLU or MoE changes parameter allocation and communication.};
\end{tikzpicture}
}
\Description{Block diagram of a decoder layer in which the residual stream passes through RMSNorm, RoPE-modified query and key vectors, grouped-query attention, another RMSNorm, and a SwiGLU or mixture-of-experts feed-forward layer.}
...
\index{RMSNorm}\index{normalization!RMSNorm}\index{pre-normalization}
...
\index{SwiGLU}\index{feed-forward network}\index{feed-forward network!SwiGLU}
...
\index{rotary position embedding}\index{rotary position embedding!scaling}\index{rotary position embedding!NTK scaling}\index{NTK scaling}
\label{sec:llama-rope}

RoPE applies a position-dependent rotation to query and key vectors before the attention dot product \cite{su2021roformer}. Pair adjacent dimensions and rotate each pair by angle $m\theta_i$ at position $m$. For a two-coordinate pair $u_{2i},u_{2i+1}$, the rotated coordinates are
\begin{equation}
  u'_{2i} = u_{2i}\cos(m\theta_i) - u_{2i+1}\sin(m\theta_i),
  \qquad
  u'_{2i+1} = u_{2i}\sin(m\theta_i) + u_{2i+1}\cos(m\theta_i).
\end{equation}
...
Baichuan 2 is a useful reminder that position handling is an architecture choice, not a family label. Its 7B model uses RoPE while its 13B model uses ALiBi, partly to study both multiplication-based and bias-based attention paths in otherwise related multilingual models \cite{yang2023baichuan2}. Such a report should not be reduced to ``LLaMA-like'' or ``not LLaMA-like.'' The positional method affects kernel compatibility, attention masks, long-context extrapolation, and what must be tested during cached decoding.

YaRN-style RoPE extension makes this tradeoff more concrete \cite{peng2023yarn}. Instead of applying one uniform interpolation rule to all rotary dimensions, it treats dimensions by frequency band. The wavelength of dimension $i$ is approximately
\begin{equation}
...
Independent LLaMA implementations make this alignment an API rather than an aside \cite{lightning2023litllama}. A common pattern is to precompute a RoPE cache and a lower-triangular mask cache up to \texttt{block\_size}; during prefill the model slices the first $T$ positions, while during cached decoding it selects rows by an explicit \texttt{input\_pos}. If the runtime also supports a fixed \texttt{max\_seq\_length}, then reaching the limit may roll the KV cache left and overwrite the final slot. That is a sliding-window policy, not just an optimization, and it should be reported because it defines which earlier tokens are no longer visible.

Several notebook-scale LLaMA implementations reveal easy mistakes. RoPE is parameterized by head dimension, not model hidden width, and one sine/cosine cache is usually shared across heads and stored as a non-trainable buffer. RoPE is applied to query and key, not value; cached keys should use the same rotated representation that attention reads at decode time. Attention scores should be divided by $\sqrt{d_h}$, not $\sqrt{d_{\mathrm{model}}}$, after heads are split. For GQA, repeating KV heads with \texttt{repeat\_interleave} is a convenient educational way to show sharing before the dot product, but the serving cache should store only $H_{\mathrm{kv}}$ heads. Physically expanding cached keys and values back to $H_q$ heads gives up much of the memory and bandwidth benefit.

\section{KV Cache and Attention Variants}
\index{KV cache}\index{KV cache!attention variants}\index{KV cache!multi-query attention}\index{KV cache!grouped-query attention}\index{multi-query attention}\index{grouped-query attention}\index{attention!multi-query attention}\index{attention!grouped-query attention}\index{multi-head attention}\index{multi-head latent attention}
...
Table~\ref{tab:attention-variants-kv} compares the attention variants by the KV-cache structure they impose.
...
\index{vocabulary}\index{byte fallback}\index{chat template}
...
\index{long-context model}\index{position extrapolation}\index{context window}
...
\index{mixture of experts}\index{mixture of experts!sparse routing}\index{mixture of experts!router}\index{mixture of experts!expert parallelism}\index{sparse MoE}\index{router}\index{expert parallelism}
...
\index{hybrid architecture}\index{attention alternative}
...
\index{long-context evaluation}\index{architecture benchmark}
...
\index{parameter count}\index{FLOP accounting}\index{active parameter}
...
\item[Latent attention] An attention design that caches or computes through compressed latent states rather than ordinary full key/value tensors.
\item[Load balancing] Router-side mechanisms that prevent a few MoE experts from receiving too many tokens while other experts remain underused.
...
\item[YaRN] A RoPE extension method that preserves high-frequency short-context behavior while interpolating lower-frequency dimensions for longer contexts.
\end{description}
...
\item For $d=4096$, compare the approximate MLP parameter counts for a GELU MLP with hidden width $4d$ and a SwiGLU MLP with hidden width $(8/3)d$ rounded up to the nearest multiple of 256.
\item Derive the relative-position property of RoPE by multiplying the transpose of $R_m$ by $R_n$ for a two-dimensional coordinate pair.
\item Compare position interpolation, NTK-aware RoPE scaling, and YaRN for a model extended from 32k to 128k context. Which parts of the report would show whether short-context behavior was preserved?
...
\item Read a DeepSeek-V3-style architecture report and separate architecture claims from systems claims. Which statements depend on MLA, MoE routing, FP8 training, custom kernels, or expert-parallel communication?
\end{enumerate}
