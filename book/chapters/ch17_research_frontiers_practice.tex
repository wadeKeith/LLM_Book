...
\paragraph{Chapter contract.} The reader should leave this chapter able to read frontier papers through the lifecycle built in earlier chapters, extract reproducible claims, map them to implementation and measurement work, and decide which ideas are durable enough to enter a curriculum or system roadmap.

\section{Why Frontier Practice Changes the Curriculum}
\index{frontier model}\index{research roadmap}
...
Figure~\ref{fig:frontier-practice-loop} summarizes the evidence loop for adding frontier claims to a durable curriculum.
...
\Description{Evidence-loop diagram from paper claim to minimal system, measurement, and system decision, with durable invariants for data, objective, architecture, optimizer, runtime, evaluation, and governance.}
...
\index{foundation model}\index{post-training pipeline}\index{deployment evaluation}
...
\index{from-scratch implementation}\index{replication}\index{ablation}
...
\index{attention alternative}\index{sparse model}\index{hybrid model}
...
Figure~\ref{fig:frontier-evidence-ladder} is the practical rule this chapter uses for deciding when a frontier result should reshape the book rather than remain on a reading list.

\begin{figure}[t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[node distance=0.55cm and 0.55cm]
\node[llmblock, text width=2.35cm] (watch) {watchlist\\claim and scope};
\node[llmblock, right=of watch, text width=2.55cm] (compare) {public comparison\\leaderboard or report};
\node[llmblock, right=of compare, text width=2.55cm] (repro) {lab reproduction\\ablation and cost};
\node[llmblock, right=of repro, text width=2.55cm] (invariant) {chapter invariant\\stable concept};
\node[llmblock, right=of invariant, text width=2.45cm] (gate) {release gate\\system checklist};

\node[llmnote, below=0.65cm of compare, text width=2.65cm] (scope) {avoid score chasing\\inspect tasks and data};
\node[llmnote, below=0.65cm of repro, text width=2.65cm] (cost) {record budget\\tokens, latency, failures};
\node[llmnote, below=0.65cm of invariant, text width=2.65cm] (teach) {teach the mechanism\\not the model name};
...
\Description{An evidence ladder progresses from watchlist claims to public comparisons, lab reproduction, chapter invariants, and release gates, with notes on scope, cost, and teaching value.}
\caption{Frontier material should move into the main text only after the claim has enough evidence to become a stable engineering invariant or release gate. The ladder reflects the book's use of CS336-style implementation evidence, public leaderboards, AILuminate-style safety benchmarks, and SWE-bench-style task harnesses \cite{stanfordcs3362026,huggingface2026leaderboards,mlcommons2026ailuminate,swebench2026verifieddataset}.}
\label{fig:frontier-evidence-ladder}
\end{figure}

\section{Reasoning and Adaptive Test-Time Compute}
\index{adaptive compute}\index{reasoning system}\index{verifier}
...
\index{data governance}\index{evaluation loop}\index{provenance}
...
\index{evidence matrix}\index{threat model}\index{publication checklist}
...
\index{reading plan}\index{research practice}
