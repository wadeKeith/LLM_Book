...
\paragraph{Chapter contract.} The reader should leave this chapter able to read or design a pretraining run card, relate optimizer and data-order decisions to the objective, budget compute and memory before launching a run, and decide what monitoring evidence is strong enough to stop or continue training.

\section{The Pretraining Problem}
\index{pretraining}\index{objective function}\index{optimizer}
...
Figure~\ref{fig:pretraining-control-loop} shows why this planning loop has to include pilot runs, dashboards, recovery, and validation from the start.

\begin{figure}[t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[node distance=7mm and 7mm]
\node[llmblock, text width=2.0cm] (plan) {Compute\\plan};
\node[llmblock, right=of plan, text width=2.1cm] (pilot) {Pilot\\runs};
\node[llmblock, right=of pilot, text width=2.1cm] (scale) {Scale law\\fit};
\node[llmblock, right=of scale, text width=2.1cm] (full) {Full\\training};
\node[llmblock, below=of full, text width=2.1cm] (dash) {Dashboard\\and alarms};
\node[llmblock, left=of dash, text width=2.1cm] (ckpt) {Checkpoint\\and resume};
\node[llmblock, left=of ckpt, text width=2.1cm] (eval) {Validation\\slices};
\draw[llmarrow] (plan) -- (pilot);
\draw[llmarrow] (pilot) -- (scale);
\draw[llmarrow] (scale) -- (full);
\draw[llmarrow] (full) -- (dash);
\draw[llmarrow] (dash) -- (ckpt);
\draw[llmarrow] (ckpt) -- (eval);
\draw[llmdash] (eval.north) .. controls +(0,1.0) and +(0,-1.0) .. node[above, font=\scriptsize] {replan} (plan.south);
\node[llmnote, below=8mm of ckpt, text width=0.78\textwidth] {Pretraining is a controlled experiment under failure. A serious run has pilot-derived expectations, token-indexed dashboards, tested recovery, and stop rules before the expensive phase begins.};
\end{tikzpicture}
}%
\Description{Control-loop diagram linking compute planning, pilot runs, scaling-law fit, full training, dashboards, checkpoint resume, validation slices, and replanning.}
\caption{Pretraining as an experimental control loop. Scaling-law planning, pilot runs, checkpoint recovery, and validation slices are not administrative extras; they determine whether the final loss curve is scientifically interpretable.}
\label{fig:pretraining-control-loop}
\end{figure}

\section{Objective Accounting and Data Order}
\index{token budget}\index{data order}\index{curriculum}\index{sample weighting}
...
\[
  ...
  +
  \sum_{j=1}^{J}\lambda_j
  \mathcal{L}_{\mathrm{MTP},j},
\]
where $\mathcal{L}_{\mathrm{MTP},j}$ predicts a future token offset and $\lambda_j$ controls how much the auxiliary task shapes the shared representation.

MTP should not be confused with a generic serving speedup. An auxiliary head may improve representations during pretraining and then be removed before deployment; a parallel-decoding head may remain in the model and change inference. The training report should therefore state whether future-token heads are used only as losses, whether they share embeddings or output heads, how their gradients are weighted, and whether they survive into the released checkpoint. Auxiliary objectives are part of objective accounting in the same sense as data mixture and masking: they change what evidence each token contributes to the run.

Table~\ref{tab:pretraining-bookkeeping} lists the bookkeeping choices that should be fixed before the run starts.
...
\index{AdamW}\index{weight decay}\index{parameter group}
...
\index{learning-rate schedule}\index{warmup}\index{batch size}
...
\begin{equation}
\begin{aligned}
\eta_t &= \eta_{\max}\frac{t}{t_w}, && \text{for } t < t_w,\\
\eta_t &= \eta_{\min} + \frac{1}{2}(\eta_{\max}-\eta_{\min})
  (1+\cos(\pi \frac{t-t_w}{t_d-t_w})),
  && \text{for } t_w \le t \le t_d,\\
\eta_t &= \eta_{\min}, && \text{for } t > t_d.
\end{aligned}
\end{equation}
...
\index{gradient clipping}\index{loss spike}\index{training instability}
...
\index{checkpoint}\index{reproducibility}\index{random seed}
...
\index{scaling law}\index{compute-optimal training}\index{Chinchilla}
...
Table~\ref{tab:pretraining-planning} collects the first-order quantities that make such planning auditable.
...
\index{validation loss}\index{early stopping}\index{evaluation cadence}
...
\section{A Quantitative Run Card}
\index{run card}\index{experiment tracking}
\label{sec:quantitative-run-card}
...
\item[FP8 scaling] The tensor scaling policy needed to map higher-precision values into FP8 ranges before low-precision matrix operations.
\item[MFU] Model FLOPs utilization, the ratio of achieved model-relevant FLOPs to hardware theoretical peak.
\item[Multi-token prediction] An auxiliary objective that predicts future tokens beyond the immediate next token, often to shape representations or support later decoding research.
...
\item Write an FP8 run-card row for one training experiment. State which tensors use E4M3 or E5M2, where accumulation stays higher precision, how scaling factors are chosen, and which BF16 or FP16 baseline validates the result.
...
\item Add an auxiliary two-token-ahead loss to the causal objective. State which hidden state predicts the future token, how you weight the auxiliary loss, and what you would remove before inference if the head is training-only.
