...
\draw[llmarrow] (model) -- node[above, font=\scriptsize] {proposed call} (runtime);
\draw[llmarrow] (runtime) -- node[above, font=\scriptsize] {authorized call} (tool);
\draw[llmarrow] (tool) -- (obs);
\draw[llmarrow] (obs) -- (audit);
\draw[llmarrow] (audit) -| (state);
\draw[llmdash] (runtime.south) -- node[right, font=\scriptsize] {deny / repair} (audit.north);
\end{tikzpicture}
